...
def main():
    ...
    def home_page():
        ...
        # st.title("HOME")
        st.markdown("""
            <h1 style="color:#c30452; text-align: center; margin-top: 10px; margin-bottom: 15px;">WEIGHT WISE</h1>
        """, unsafe_allow_html=True)
    ...
    def Prediction():
        ...
        def main():
            ...
            with col2:
                ...
                all_diets_data_path = os.path.join(current_dir, 'data', 'All_Diets.csv')

                df = pd.read_csv(all_diets_data_path)
    ...
    def Help():
        ...
        This application has multiple pages: Home, Insights, Prediction, Blog, YouTube, and Help. You assist users by explaining the app’s features, answering health and obesity-related questions, providing model prediction insights, and guiding users on technical or content-related issues.

        Key points about the Weight Wise app:

        - The app is built with Streamlit, featuring multiple pages navigated via a menu (e.g. streamlit_option_menu).
        - It uses a pre-trained OvO Decision Tree model (ovo_decision_tree_model.pkl) to predict obesity levels based on user input.
        - The pipeline module (ObesityType_pipeline) contains core functions like feature engineering, including BMI, BMR, DCI, IDEAL_WEIGHT calculations.
        - Categorical variables are encoded with LabelEncoder; feature columns are strictly ordered for model input.
        - The Home page provides app intro, WHO data, project overview, and dataset explanations.
        - Insights page displays visualizations (histograms, density plots, gender vs obesity), model scores (OvO Decision Tree highest with 0.974 accuracy), and feature importance.
        - Prediction page lets users input personal data, calculates BMI, BMR, DCI, ideal weight, and shows prediction results with explanations.
        - A nutrition recommendation module filters foods from All_Diets.csv by user’s diet type, cuisine, and calorie limit (using DCI).
        - Food alternatives are suggested based on cosine similarity of nutrient ratios.
        - Blog section fetches and shows curated health content using requests and BeautifulSoup; YouTube page searches and displays relevant videos via Google API.
        - Help page supports sending emails via SMTP for feedback, plus fun UI elements like a music+confetti button.
        - Images in the app are base64 encoded and embedded via HTML for smooth Streamlit display.

        Your capabilities:

        1. Explain app sections and their purpose, e.g. what users can do on Prediction, Insights, Blog, or YouTube pages.
        2. Interpret user inputs and model predictions, providing clear, friendly explanations of BMI, BMR, obesity risk, and ideal weight calculations.
        3. Answer general questions about obesity, healthy diets, and exercise with evidence-based but accessible language.
        4. Offer troubleshooting tips for common issues (e.g. API YouTube recommendations, blog image loading, email sent). 
        5. Recommend useful blog articles or YouTube videos based on user interests.
        6. Provide guidance on using the app’s interactive features, like navigation, input forms, or the confetti music button.
        7. Maintain a supportive, positive tone encouraging healthy lifestyle choices and curiosity.

        Example conversation snippets you should handle well:

        - "What does BMI mean in this app?"
        - "How does the OvO Decision Tree predict obesity?"
        - "Why are some foods filtered out in the diet recommendation?"

        - "Can you suggest exercises to reduce belly fat?"
        - "Why are blog images sometimes not showing?"
        - "How does the app calculate daily calorie needs?"
        - "What kind of emails can I send from the Help page?"
        - "How do I navigate between Home and Prediction pages?"

        Always tailor your answers based on the context of the Weight Wise app and avoid generic responses unrelated to this application.

        If you don’t know the answer, politely say so and suggest where the user can find more info.
        """
        )

        def weight_wise_chatbot():
            st.markdown("""
                            <h2 style="color:#c30452;"> 🧠 Weight Wise Chatbot</h2>
                        """, unsafe_allow_html=True)

            if "chat_history" not in st.session_state:
                st.session_state.chat_history = []

            user_input = st.text_input("Sormak istediğiniz bir şey var mı?", key="chatbot_input")

            if user_input:
                with st.spinner("Yanıt yazılıyor..."):
                    try:
                        full_prompt = BASE_INSTRUCTION + "\nKullanıcı sorusu: " + user_input
                        response = model.generate_content(full_prompt)
                        bot_reply = response.text.strip()
                        st.session_state.chat_history.append(("👤 Siz", user_input))
                        st.session_state.chat_history.append(("🤖 Asistan", bot_reply))
                    except Exception as e:
                        error_msg = f"Hata oluştu: {str(e)}"
                        st.session_state.chat_history.append(("❌ Hata", error_msg))
        ...
        # Define the relative path to your audio file
        audio_file_name = 'Earth_Wind_Fire_September.mp3'
        audio_file_path = os.path.join(current_dir, 'data', audio_file_name)
        ...
        st.markdown("""
            <div style="text-align:center;">
                <h3 style="color:#c30452; padding-left:20px;">Please connect with us 🐝</h3>
                <p style="font-size:20px; color:#c30452;">
                    <b><a href="mailto:weightwisehelp24@gmail.com" style="color:#c30452; text-decoration:none;">E-mail: weightwisehelp24@gmail.com</a></b>
                </p>
            </div>
            """, unsafe_allow_html=True)
